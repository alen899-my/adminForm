...
export default function Page() {
          ...
          if (data.success && data.lead) {
            ...
            if (l.attachments) {
              l.attachments.forEach(att => {
                if (att.fieldname === 'companyLogo') fileMap.logoCompany = att.filename;
                if (att.fieldname === 'clientLogo') fileMap.logoClient = att.filename;
                if (att.fieldname === 'vatCertificate') fileMap.vatCertificate = att.filename;
                if (att.fieldname === 'tradeLicense') fileMap.tradeLicense = att.filename;
              });
              setExistingFiles(prev => ({ ...prev, ...fileMap }));
            }
  ...
  // --- MODIFIED FILE UPLOAD COMPONENT TO SHOW EXISTING FILES ---
  const FileUploadBlock = ({ label, name, accept, file, setFormData, existingFileName }) => {
        ...
        >
          <span className="truncate">
            {file 
              ...
              : (existingFileName ? `Current: ${existingFileName}` : "No file chosen")
            }
          </span>
