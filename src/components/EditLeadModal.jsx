...
import {
  ...
  Upload, UserCog, FileText, CheckCircle, X
} from "lucide-react";
  ...
  useEffect(() => {
    if (leadData) {
      ...
      setFormData((prev) => ({
        ...
        // Reset file inputs to null on edit open
        logoCompany: null,
        logoClient: null,
        vatCertificate: null,
        tradeLicense: null,
    ...
    return (
      <div className="border rounded-lg p-2 bg-gray-50 flex flex-col gap-2">
        ...
        <div className="flex flex-col gap-1">
            {/* Show Existing File info if available and no new file selected */}
      ...
      <div className="w-full max-w-4xl bg-white rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="bg-blue-600 p-4 flex justify-between items-center text-white shrink-0">
            <h2 className="text-lg font-semibold flex items-center gap-2">
                <UserCog className="w-5 h-5" /> Edit Lead Details
            </h2>
        ...
        <div className="overflow-y-auto p-6 md:p-8">
            ...
            <div className="flex gap-1 overflow-x-auto no-scrollbar scroll-smooth rounded-lg border border-gray-200 bg-white p-1">
                {[
                ...
                return (
                    ...
                    className={`
                        flex items-center gap-2 px-3 sm:px-4 py-2 text-xs sm:text-sm font-medium whitespace-nowrap rounded-md transition-all
                        ${isActive 
            ...
            {/* Error Message */}
            {wizardError && (
            <div className="text-red-600 text-sm mb-3 bg-red-100 border border-red-300 px-3 py-2 rounded-lg text-center">
                ⚠️ {wizardError}
            </div>
            )}
            ...
            {/* Success Message */}
            {isSubmitted ? (
                <div className="bg-green-100 text-green-800 p-8 rounded-lg text-center flex flex-col items-center gap-3">
                ...
                <>
                    ...
                    <div className="space-y-4 animate-in fade-in slide-in-from-right-4">
                         <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="number" name="lobbies" value={formData.lobbies} onChange={handleChange} className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" />
                            </div>
                            <div>
                                <label className="text-sm font-medium text-gray-900">Key Rooms</label>
                                <input type="number" name="keyRooms" value={formData.keyRooms} onChange={handleChange} className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" />
                            </div>
                            <div>
                                <label className="text-sm font-medium text-gray-900">Distance</label>
                                <input type="text" name="distance" value={formData.distance} onChange={handleChange} className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" />
                            </div>
                            {/* Radios */}
                            {['supervisorUser', 'validationUser', 'reportUser'].map(field => (
                                <div key={field} className="border rounded-lg p-3 bg-gray-50">
                                    ...
                                    <div className="flex items-center gap-4">
                                        <label className="flex items-center gap-2 cursor-pointer text-sm"><input type="radio" name={field} value="yes" checked={formData[field] === "yes"} onChange={handleChange} className="text-blue-600 focus:ring-blue-500" /> Yes</label>
                                        <label className="flex items-center gap-2 cursor-pointer text-sm"><input type="radio" name={field} value="no" checked={formData[field] === "no"} onChange={handleChange} className="text-blue-600 focus:ring-blue-500" /> No</label>
                                    </div>
                                </div>
                            ))}
                         </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="p-3 bg-gray-50 rounded-lg border">
                                ...
                                <div className="flex flex-col gap-2">
                                    <label className="flex gap-2 text-sm"><input type="radio" name="ticketType" value="pre-printed" checked={formData.ticketType === "pre-printed"} onChange={handleChange} className="text-blue-600" /> Pre-printed</label>
                                    <label className="flex gap-2 text-sm"><input type="radio" name="ticketType" value="system-generated" checked={formData.ticketType === "system-generated"} onChange={handleChange} className="text-blue-600" /> System Generated</label>
                                </div>
                            </div>
                            <div className="p-3 bg-gray-50 rounded-lg border">
                                <label className="block text-sm font-medium mb-2">Fee Type</label>
                                <div className="flex gap-4 flex-wrap">
                                    <label className="flex gap-2 text-sm"><input type="radio" name="feeType" value="fixed" checked={formData.feeType === "fixed"} onChange={handleChange} className="text-blue-600" /> Fixed</label>
                                    <label className="flex gap-2 text-sm"><input type="radio" name="feeType" value="hourly" checked={formData.feeType === "hourly"} onChange={handleChange} className="text-blue-600" /> Hourly</label>
                                </div>
                            </div>
                            <div className="md:col-span-2">
                                <label className="text-sm font-medium">Ticket Prices</label>
                                <textarea rows={2} name="ticketPricing" value={formData.ticketPricing} onChange={handleChange} className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" />
                            </div>
                        </div>
                    </div>
                    )}
                    ...
                    <div className="space-y-4 animate-in fade-in slide-in-from-right-4">
                        <div className="grid grid-cols-1 gap-4">
                            <div>
                                <label className="text-sm font-medium">Driver Count</label>
                                <input type="number" name="driverCount" value={formData.driverCount} onChange={handleChange} className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" />
                            </div>
                            <div>
                                <label className="text-sm font-medium">Driver List</label>
                                <textarea rows={5} name="driverList" value={formData.driverList} onChange={handleChange} className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none font-mono text-sm" />
                            </div>
                        </div>
                    </div>
                    )}
                    ...
                    <div className="space-y-4 animate-in fade-in slide-in-from-right-4">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="md:col-span-2">
                                ...
                                <input type="text" name="adminName" value={formData.adminName} onChange={handleChange} className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none ${errors.adminName ? "border-red-500" : ""}`} />
                            </div>
                            <div>
                                <label className="text-sm font-medium">Email *</label>
                                <input type="email" name="adminEmail" value={formData.adminEmail} onChange={handleChange} className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none ${errors.adminEmail ? "border-red-500" : ""}`} />
                            </div>
                            <div>
                                <label className="text-sm font-medium">Phone *</label>
                                <input type="tel" name="adminPhone" value={formData.adminPhone} onChange={handleChange} className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none ${errors.adminPhone ? "border-red-500" : ""}`} />
                            </div>
                        </div>
                    </div>
                    )}
                    ...
                    <div className="space-y-4 animate-in fade-in slide-in-from-right-4">
                        ...
                        <div>
                            ...
                            <textarea rows={2} name="documentSubmitMethod" value={formData.documentSubmitMethod} onChange={handleChange} className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" />
                        </div>
                    </div>
                    )}
                </>
        ...
        {!isSubmitted && (
             ...
             {currentStep > 1 ? (
                 <button onClick={() => setCurrentStep(prev => prev - 1)} className="px-4 py-2 text-gray-600 hover:text-gray-900">
                     ← Back
                 </button>
             ) : (
                 <div /> 
